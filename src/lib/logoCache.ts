 ...
 * Always return null to use ESPN URLs for consistent high quality
 */
export function getCachedLogoUrl(teamId: string, size: number): string | null {
  // Always use ESPN URLs for consistent high quality
  // Local cache has quality issues with small sizes
  return null;
}
