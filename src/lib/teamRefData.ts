...
export async function getTeamRefData(): Promise<TeamRefData[]> {
  ...
  // Try to fetch from Google Sheets using the same pattern as other sheets
  try {
    console.log('📋 Attempting to fetch team reference data from Google Sheets...');
    
    // Use the same URL format as other successful Google Sheets
    ...
    console.log('📋 Using fallback team reference data due to Google Sheets error');
